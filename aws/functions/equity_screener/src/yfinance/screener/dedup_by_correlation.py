...
def greedy_dedup(symbols: list[str], corr: pd.DataFrame, threshold: float) -> dict[str, str | None]:
    kept: list[str] = []
    dropped_due_to: dict[str, str | None] = {}
    for s in symbols:
        dropped = False
        for k in kept:
...
def cluster_dedup(symbols: list[str], corr: pd.DataFrame, threshold: float) -> dict[str, str | None]:
    try:
        import numpy as np
...
def main():
    ...
    p.add_argument(
        "--threshold", type=float, default=0.85, help="correlation threshold (abs) to consider symbols as duplicates"
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="use only the latest N trading rows from downloaded prices (overrides period when provided)",
    )
    p.add_argument(
        "--transform", choices=["NONE", "LOG"], default="NONE", help="return transformation to compute (NONE or LOG)"
    )
    args = p.parse_args()
